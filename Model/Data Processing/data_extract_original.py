...
def extract_summary(json_data, summary_key, entity_dict):
	...
	for game in json_data:
		...
		result_tokens = nltk.tokenize.word_tokenize(' '.join(result), language='english')
		#result_tokens = ' '.join(result)
...
def extract_links(json_data, summary_list, entity_list, verbose=False):
	...
	for game, summary, entities in zip(json_data, summary_list, entity_list):
		...
		summary_sents = nltk.tokenize.sent_tokenize(summary_text)
		if verbose:
...
if __name__ == '__main__':
	...
	json_data = json.load(open("../testFIle.json", 'r'))
	...
	links = extract_links(json_data, summary_list, game_entity_list, True)
	assert len(table_list) == len(links)
	final_links = convert_links(json_data, links)

	table_labels_list, summary_labels_list = extract_labels(table_list, final_links, summary_list, True)

	with open("NewTest.gtable", "w") as outf:
		for game in table_list:
			assert all([len(item.split('|')) == 4 for item in game])
			outf.write("{}\n".format(' '.join(game)))
	outf.close()

	with open("NewTest.summary", "w") as outf:
		for summary in summary_list:
			outf.write("{}\n".format(' '.join(summary)))
	outf.close()

	with open("NewTest.orig_summary", "w") as outf:
		for game in json_data:
			outf.write("{}\n".format(' '.join(game[summary_key])))
	outf.close()

	with open("NewTest.links", "w") as outf:
		for links in final_links:
			out_line = ' '.join(sorted(links, key=lambda x: int(x[:x.index(':')])))
			outf.write("{}\n".format(out_line))
	outf.close()

	with open("NewTest.gtable_label", 'w') as outf:
		for table_labels in table_labels_list:
			outf.write("{}\n".format(' '.join(table_labels)))
	outf.close()

	with open("NewTest.summary_label", 'w') as outf:
